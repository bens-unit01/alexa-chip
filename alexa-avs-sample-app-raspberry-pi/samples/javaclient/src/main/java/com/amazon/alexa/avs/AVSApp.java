...
public class AVSApp implements ExpectSpeechListener, RecordingRMSListener,
    ...
    private TwoWaySerialComm serialPort = null;  
    private String accessToken;

    static boolean motorDirection = false; 
...
//        setVisible(true);
      ...
      System.out.println("actionPerformed ... bloc if");
      ...
      serialPort.write((byte) Commands.START_BYTE); 
      serialPort.write((byte) Commands.WAKE_WORD_DETECTED); 
      serialPort.write((byte) 0x00); 
      serialPort.write((byte) 0x00); 
      ...
      controller.startRecording(rmsListener, requestListener);
	      ...
	      System.out.println("actionPerformed ... bloc else");
	      ...
	      Thread thread = new Thread() {
		      @Override
			      public void run() {
                                      ...
                                      byte direction = (motorDirection)? (byte) 0:(byte)1; 
				      for(int i =0; i < 15; i++) {
                                              serialPort.write((byte) Commands.START_BYTE); 
					      serialPort.write((byte) Commands.DRIVE); 
					      serialPort.write((byte) direction); 
					      serialPort.write((byte) 0x00); 
    ...
    public void onExpectSpeechDirective() {
        Thread thread = new Thread() {
            ...
            public void run() {
                ...
                while (buttonState != ButtonState.START
...
//        }
//        System.out.println("displayRegCode return ...");
    }

    @Override
    public synchronized void onAccessTokenReceived(String accessToken) {
    	System.out.println("onAccessTokenReceived ...");
        serialPort.write((byte) Commands.START_BYTE); 
	serialPort.write((byte) Commands.RGB_CTRL_COMMAND); 
	serialPort.write((byte) Commands.RGB_SOLID_BLUE); 
	serialPort.write((byte) 0x00); 
    ...
    public synchronized void onWakeWordDetected() {
		    if (buttonState == ButtonState.START) { // if in idle mode
			    log.info("[AVSApp:onWakeWordDetected] ------   Wake Word was detected");
