...
COMMON_NAME="52.8.126.217" openssl req -new -x509 -days 365 -key certs/ca/ca.key -out certs/ca/ca.crt -config ssl.cnf -sha256

# Create the Client KeyPair for the Device Code
openssl genrsa -out certs/client/client.key 2048
COMMON_NAME="wow_robot_01:123456" openssl req -new -key certs/client/client.key -out certs/client/client.csr -config ssl.cnf -sha256
...
COMMON_NAME="52.8.126.217" openssl req -new -key certs/server/node.key -out certs/server/node.csr -config ssl.cnf -sha256
...
COMMON_NAME="52.8.126.217" openssl req -new -key certs/server/jetty.key -out certs/server/jetty.csr -config ssl.cnf -sha256
COMMON_NAME="52.8.126.217" openssl x509 -req -days 365 -in certs/server/jetty.csr -CA certs/ca/ca.crt -CAkey certs/ca/ca.key -set_serial 03 -out certs/server/jetty.crt -extensions v3_req -extfile ssl.cnf -sha256
